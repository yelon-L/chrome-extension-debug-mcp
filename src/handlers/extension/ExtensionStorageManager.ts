...
// 简化日志函数
const log = (...args: any[]) => console.error('[ExtensionStorageManager]', ...args);
...
import { 
  ...
  InspectExtensionStorageResponse,
...
export class ExtensionStorageManager {
   ...
   */
  async inspectExtensionStorage(args: InspectExtensionStorageArgs): Promise<InspectExtensionStorageResponse> {
    try {
      ...
      if (!targetContext) {
        throw new McpError(ErrorCode.InvalidParams, 
          `Cannot find accessible context for extension ${extensionId} to inspect storage`);
      }
      ...
      if (!switchResult.success) {
        throw new McpError(ErrorCode.InternalError, 
          `Failed to switch to context for storage inspection`);
      }
      ...
      // 4. 读取存储数据（带重试机制）
      const storageData = await this.readExtensionStorageDataWithRetry(
        extensionId, 
        storageTypes, 
        keys, 
        switchResult.sessionId
      );

      const response: InspectExtensionStorageResponse = {
        extensionId,
        storageData,
   ...
   * 查找可访问存储的上下文
   */
  private async findStorageAccessibleContext(extensionId: string): Promise<ContextTarget | null> {
    ...
    for (let i = 0; i < maxPagesToCheck; i++) {
      ...
      try {
        const hasContentScript = await this.verifyContentScript(extensionId, (target as any).tabId);
        if (hasContentScript) {
          return {
            extensionId,
        ...
        log(`Skip checking tab ${(target as any).tabId} due to error:`, e);
        continue;
      }
    }
   ...
   * 验证内容脚本存在 - 带超时控制
   */
  private async verifyContentScript(extensionId: string, tabId: string): Promise<boolean> {
    ...
    try {
      ...
      const verifyPromise = page.evaluate((extId) => {
        ...
        const hasExtensionScript = scripts.some(script => 
          (script as HTMLScriptElement).src.includes(extId)
        );
        
        const hasExtensionMarker = document.documentElement.getAttribute(`data-extension-${extId}`) !== null;
        
        return hasExtensionScript || hasExtensionMarker || (window as any)[`ext_${extId}_loaded`];
      }, extensionId);
  ...
  private async detectStorageCapabilities(
    ...
    sessionId?: string
  ): Promise<{ canRead: boolean; canWrite: boolean; canClear: boolean }> {
    const cdpClient = this.chromeManager.getCdpClient();
   ...
   */
  private async wakeUpServiceWorker(extensionId: string, sessionId?: string): Promise<void> {
    const cdpClient = this.chromeManager.getCdpClient();
    if (!cdpClient) return;
   ...
   * 带重试机制的存储数据读取
   */
  private async readExtensionStorageDataWithRetry(
    extensionId: string,
    storageTypes: string[],
    keys?: string[],
    sessionId?: string,
    ...
    return storageTypes.map(type => ({
      type: type as 'local' | 'sync' | 'session' | 'managed',
      data: {},
      quota: undefined,
   ...
   * 读取扩展存储数据
   */
  private async readExtensionStorageData(
    extensionId: string,
    storageTypes: string[],
    keys?: string[],
    sessionId?: string
    ...
    const storageData: ExtensionStorageData[] = [];
      ...
      try {
        ...
        if (evalResult.result?.value) {
          ...
          if (storageResult.error) {
            ...
            // 添加空的存储数据记录错误
            storageData.push({
              type: storageType as 'local' | 'sync' | 'session' | 'managed',
              data: {},
   ...
   * 启动存储监控（未来功能）
   */
  async startStorageWatcher(extensionId: string, storageTypes: string[]): Promise<void> {
    // TODO: 实现存储变更监控
   ...
   * 停止存储监控（未来功能）
   */
  async stopStorageWatcher(extensionId: string): Promise<void> {
    // TODO: 实现停止监控
