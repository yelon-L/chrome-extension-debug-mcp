...
export class ExtensionMessageTracker {
  ...
  private apiCalls: ExtensionAPICall[] = [];
  ...
  async monitorExtensionMessages(args: MonitorExtensionMessagesArgs) {
    ...
    try {
      ...
      // 注入监控脚本到每个扩展上下文
      const monitoringPromises = extensionTargets.map((target: any) => 
        this.injectMessageMonitor(target.targetId, extensionId, messageTypes, includeResponses)
      );
   ...
   */
  private async injectMessageMonitor(
    targetId: string,
    extensionId: string,
    messageTypes: string[],
    includeResponses: boolean
  ...
  async trackExtensionAPICalls(args: TrackExtensionAPICallsArgs) {
    ...
    try {
      ...
      // 注入API追踪脚本
      const trackingPromises = extensionTargets.map((target: any) => 
        this.injectAPITracker(target.targetId, extensionId, apiCategories, includeResults)
      );
   ...
   * 注入API追踪脚本
   */
  private async injectAPITracker(
    targetId: string,
    extensionId: string,
