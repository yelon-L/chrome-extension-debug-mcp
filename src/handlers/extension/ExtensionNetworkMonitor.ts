...
export class ExtensionNetworkMonitor {
  ...
  private monitoringStartTime: Map<string, number> = new Map();
   ...
   * 开始监控扩展网络请求
   */
  async trackExtensionNetwork(
    args: TrackExtensionNetworkArgs
   ...
   * 处理响应事件
   */
  private handleResponse(response: HTTPResponse, extensionId: string): void {
    try {
      ...
      if (networkRequest) {
        ...
        networkRequest.protocol = (response as any).protocol?.() || undefined;
   ...
   * 处理请求失败事件
   */
  private handleRequestFailed(request: HTTPRequest, extensionId: string): void {
    try {
      const requests = this.requests.get(extensionId) || [];
      const networkRequest = requests.find(r => r.url === request.url() && r.timing.endTime === 0);
   ...
   * 按资源类型分组
   */
  private groupByResourceType(requests: NetworkRequest[]): Record<string, number> {
   ...
   * 按域名分组
   */
  private groupByDomain(requests: NetworkRequest[]): Record<string, number> {
    const groups: Record<string, number> = {};
    requests.forEach(req => {
      try {
   ...
   * 按请求方法分组
   */
  private groupByMethod(requests: NetworkRequest[]): Record<string, number> {
    const groups: Record<string, number> = {};
    requests.forEach(req => {
   ...
   * 提取第三方域名
   */
  private extractThirdPartyDomains(requests: NetworkRequest[]): string[] {
   ...
   * 生成网络优化建议
   */
  private generateNetworkRecommendations(stats: {
    ...
    if (stats.totalRequests > 100) {
      recommendations.push(
        `⚠️ 请求数量过多（${stats.totalRequests}个），考虑合并请求、使用缓存或延迟加载`
      );
    } else if (stats.totalRequests > 50) {
      recommendations.push(
        `💡 请求数量较多（${stats.totalRequests}个），可以考虑优化请求策略`
      );
    }
    ...
    if (dataMB > 10) {
      recommendations.push(
        `⚠️ 数据传输量较大（${dataMB.toFixed(2)}MB），考虑数据压缩或减少传输内容`
      );
    } else if (dataMB > 5) {
      recommendations.push(
        `💡 数据传输量（${dataMB.toFixed(2)}MB）可以进一步优化`
      );
    }
    ...
    if (stats.averageRequestTime > 2000) {
      recommendations.push(
        `⚠️ 平均请求时间过长（${stats.averageRequestTime.toFixed(0)}ms），检查网络连接或服务器性能`
      );
    } else if (stats.averageRequestTime > 1000) {
      recommendations.push(
        `💡 平均请求时间较长（${stats.averageRequestTime.toFixed(0)}ms），可以考虑优化`
      );
    }
    ...
    if (stats.failedRequests > 0) {
      recommendations.push(
        `⚠️ 检测到${stats.failedRequests}个失败请求，建议检查错误原因并添加重试机制`
      );
    }
    ...
    if (stats.suspiciousRequests > 0) {
      recommendations.push(
        `⚠️ 检测到${stats.suspiciousRequests}个可疑请求，建议审查其必要性和安全性`
      );
    }
    ...
    if (cacheRate < 20 && stats.totalRequests > 10) {
      recommendations.push(
        `💡 缓存使用率较低（${cacheRate.toFixed(1)}%），考虑增加缓存策略`
      );
    }
    ...
    if (stats.thirdPartyDomains > 10) {
      recommendations.push(
        `💡 请求了${stats.thirdPartyDomains}个不同的域名，考虑减少第三方依赖`
      );
    }
   ...
   * 简化请求对象（去除大数据）
   */
  private simplifyRequest(request: NetworkRequest): NetworkRequest {
    return {
      ...request,
      requestHeaders: {},
      responseHeaders: {},
  ...
  analyzeNetworkPattern(args: {
    ...
    patterns: {
      frequentDomains: Array<{ domain: string; count: number; percentage: number }>;
      resourceTypeDistribution: Array<{ type: string; count: number; size: number; percentage: number }>;
      methodDistribution: Array<{ method: string; count: number }>;
      statusDistribution: Array<{ status: number; count: number }>;
      timelineAnalysis: {
        peakTime: string;
        avgRequestsPerMinute: number;
        busiestPeriod: { start: number; end: number; count: number };
      };
    };
    issues: Array<{
