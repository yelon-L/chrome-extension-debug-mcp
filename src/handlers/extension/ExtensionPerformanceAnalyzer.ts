...
export class ExtensionPerformanceAnalyzer {
  ...
  private calculateMetrics(events: TraceEvent[]): PerformanceMetrics {
    ...
    // 计算各类时间
    const scriptEvents = validEvents.filter(e => 
      e.name === 'EvaluateScript' || e.name === 'v8.compile' || e.name === 'v8.run'
    );
    const layoutEvents = validEvents.filter(e => 
      e.name === 'Layout' || e.name === 'UpdateLayoutTree'
    );
    const paintEvents = validEvents.filter(e => 
      e.name === 'Paint' || e.name === 'CompositeLayers'
    );
  ...
  private calculateCoreWebVitals(events: TraceEvent[]): CoreWebVitals {
    ...
    const cls = layoutShiftEvents.reduce((sum, e) => 
  ...
  private calculateImpact(
    deltaMetrics: PerformanceMetrics,
  ...
  private generateRecommendations(
    ...
    if (delta.cpuUsage > 10) {
      recommendations.push(
        `⚠️ CPU使用率增加${delta.cpuUsage.toFixed(1)}%，建议优化JavaScript执行逻辑，减少同步操作`
      );
    } else if (delta.cpuUsage > 5) {
      recommendations.push(
        `💡 CPU使用率增加${delta.cpuUsage.toFixed(1)}%，考虑使用Web Workers处理计算密集型任务`
      );
    }
    ...
    if (delta.memoryUsage > 50) {
      recommendations.push(
        `⚠️ 内存使用增加${delta.memoryUsage.toFixed(1)}MB，检查是否存在内存泄漏，及时清理不需要的对象`
      );
    } else if (delta.memoryUsage > 20) {
      recommendations.push(
        `💡 内存使用增加${delta.memoryUsage.toFixed(1)}MB，考虑优化数据结构，减少内存占用`
      );
    }
    ...
    if (delta.scriptEvaluationTime > 500) {
      recommendations.push(
        `⚠️ 脚本执行时间增加${delta.scriptEvaluationTime.toFixed(0)}ms，建议分割大型脚本或延迟加载`
      );
    }
    ...
    if (impact.pageLoadDelay > 500) {
      recommendations.push(
        `⚠️ 页面加载延迟${impact.pageLoadDelay.toFixed(0)}ms，考虑将扩展初始化移至页面加载完成后`
      );
    } else if (impact.pageLoadDelay > 200) {
      recommendations.push(
        `💡 页面加载延迟${impact.pageLoadDelay.toFixed(0)}ms，可以进一步优化启动性能`
      );
    }
    ...
    if (impact.cwvImpact.lcp > 500) {
      recommendations.push(
        `⚠️ LCP增加${impact.cwvImpact.lcp.toFixed(0)}ms，扩展可能阻塞了关键内容的渲染`
      );
    }
  ...
  private calculateImpactLevel(
    ...
    // CPU影响评分
    if (delta.cpuUsage > 20) score += 3;
    else if (delta.cpuUsage > 10) score += 2;
    else if (delta.cpuUsage > 5) score += 1;
    
    // 内存影响评分
    if (delta.memoryUsage > 100) score += 3;
    else if (delta.memoryUsage > 50) score += 2;
    else if (delta.memoryUsage > 20) score += 1;
    
    // 加载延迟评分
    if (impact.pageLoadDelay > 1000) score += 3;
    else if (impact.pageLoadDelay > 500) score += 2;
    else if (impact.pageLoadDelay > 200) score += 1;
    
    // LCP影响评分
    if (impact.cwvImpact.lcp > 1000) score += 3;
    else if (impact.cwvImpact.lcp > 500) score += 2;
    else if (impact.cwvImpact.lcp > 200) score += 1;
