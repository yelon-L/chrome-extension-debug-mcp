...
import { 
  ...
  ExtensionPageTestResult,
  ExtensionTestCaseResult,
  ExtensionPagePerformance,
  ExtensionPerformanceImpact,
  BatchTestOptions
...
export class ExtensionTestHandler {
   ...
   * 获取扩展基础信息
   */
  private async getExtensionInfo(extensionId: string, result: ExtensionTestResult): Promise<void> {
    try {
      // 尝试从targets中获取扩展信息
      ...
      const { targetInfos } = await cdpClient.send('Target.getTargets') as { targetInfos: any[] };
      
      const extensionTarget = targetInfos.find(target => 
        target.url && target.url.includes(extensionId)
      );
  ...
  private async executeBatchTests(
    args: TestExtensionOnMultiplePagesArgs, 
    timeout: number, 
    concurrency: number
  ): Promise<ExtensionPageTestResult[]> {
    
    const results: ExtensionPageTestResult[] = [];
    const testPromises: Promise<ExtensionPageTestResult>[] = [];
   ...
   * 等待可用的并发槽位
   */
  private async waitForAvailableSlot(semaphore: (Promise<any> | null)[]): Promise<number> {
    while (true) {
  ...
  private async testSinglePage(
    ...
    const pageResult: ExtensionPageTestResult = {
      url,
      tabId: '',
      status: 'failed',
      startTime,
    ...
    try {
      ...
      const { targetId } = await cdpClient.send('Target.createTarget', { url }) as { targetId: string };
      tabId = targetId;
      ...
      if (args.testCases && args.testCases.length > 0) {
        pageResult.testCaseResults = await this.executeTestCases(
          targetId, 
          args.extensionId, 
          args.testCases
        );
   ...
   * 等待页面加载完成
   */
  private async waitForPageLoad(targetId: string, timeout: number): Promise<void> {
    const cdpClient = this.chromeManager.getCdpClient();
  ...
  private async executeTestCases(
    ...
    extensionId: string, 
    testCases: any[]
  ): Promise<ExtensionTestCaseResult[]> {
    
    const results: ExtensionTestCaseResult[] = [];
    ...
    for (const testCase of testCases) {
      ...
      try {
        ...
        const testResult: ExtensionTestCaseResult = {
   ...
   * 执行默认测试
   */
  private async executeDefaultTests(targetId: string, extensionId: string): Promise<ExtensionTestCaseResult[]> {
    return [
      {
   ...
   * 检查内容脚本注入状态
   */
  private async checkContentScriptInjection(targetId: string): Promise<any> {
    try {
      const cdpClient = this.chromeManager.getCdpClient();
      if (!cdpClient) {
   ...
   * 执行自定义脚本
   */
  private async executeCustomScript(targetId: string, script: string): Promise<any> {
    try {
      const cdpClient = this.chromeManager.getCdpClient();
      if (!cdpClient) {
   ...
   * 创建失败页面结果
   */
  private createFailedPageResult(url: string, error: string): ExtensionPageTestResult {
    const now = Date.now();
    return {
      url,
      tabId: '',
   ...
   * 生成测试摘要
   */
  private generateTestSummary(result: ExtensionTestResult): void {
   ...
   * 分析性能影响
   */
  private analyzePerformanceImpact(pageResults: ExtensionPageTestResult[]): ExtensionPerformanceImpact {
