...
export class ExtensionContentScript {
  ...
  async injectContentScript(params: {
    ...
    try {
      ...
      const targetTabs = tabId ? tabs.filter((tab: any) => tab.id === tabId) : tabs;
  ...
  async contentScriptStatus(params: { 
  ...
  }): Promise<{ results: ContentScriptStatusResult[] }> {
    ...
    try {
      ...
      const targetTabs = checkAllTabs ? tabs : tabs.filter((tab: any) => !tabId || tab.id === tabId);
        ...
        try {
          ...
          if (analysis) {
            results.push({
              ...
              title: tab.title || 'Unknown',
              extensionId,
              injectionStatus: analysis.injectionStatus as any,
              details: {
   ...
   */
  async hasContentScriptInTab(extensionId: string, tabId: string): Promise<boolean> {
    try {
   ...
   * 验证内容脚本在特定页面中的存在
   */
  private async verifyContentScript(page: Page, tabId: string): Promise<ContentScriptDetectionResult | null> {
    try {
      // 预检查页面有效性
      if (!(await this.isPageValid(page))) {
      ...
      const result = await page.evaluate(() => {
        const result = {
          ...
          conflicts: [] as any[],
          extensionMarkers: [] as string[],
          globalVariables: [] as string[],
          detectedExtensionId: null as string | null
        };

        // 检查扩展标记
        const allElements = document.querySelectorAll('*');
        const extElements: Element[] = [];
        ...
        const extensionScripts = scripts.map(s => (s as HTMLScriptElement).src);
   ...
   * 分析标签页中的内容脚本
   */
  private async analyzeContentScriptInTab(page: Page, tabId: string, extensionId?: string): Promise<ContentScriptDetectionResult | null> {
    try {
      // 首先检查页面有效性
      if (!(await this.isPageValid(page))) {
