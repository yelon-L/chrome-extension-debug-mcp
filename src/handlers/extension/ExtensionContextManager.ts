...
// 简化日志函数
const log = (...args: any[]) => console.error('[ExtensionContextManager]', ...args);
...
import { 
  ...
  ListExtensionContextsResponse,
  SwitchExtensionContextArgs,
  SwitchExtensionContextResponse,
  ExtensionContext,
  ExtensionContexts,
  ExtensionBackgroundContext,
  ExtensionContentScriptContext
} from '../../types/index.js';
...
export class ExtensionContextManager {
   ...
   * Week 2: 列出扩展的所有上下文
   */
  async listExtensionContexts(args: ListExtensionContextsArgs): Promise<ListExtensionContextsResponse> {
    try {
      ...
      // 为每个扩展构建上下文信息
      const extensions: ExtensionContext[] = [];
      let totalContexts = 0;
      ...
      for (const [extId, targets] of extensionMap) {
        ...
        // 更新统计
        if (extContext.contexts.background?.active) summary.activeBackgrounds++;
        summary.totalContentScripts += extContext.contexts.contentScripts.length;
        if (extContext.contexts.popup?.open) summary.openPopups++;
        if (extContext.contexts.options?.open) summary.openOptions++;
        if (extContext.contexts.devtools) summary.activeDevtools += extContext.contexts.devtools.length;
   ...
   */
  async switchExtensionContext(args: SwitchExtensionContextArgs): Promise<SwitchExtensionContextResponse> {
    try {
      ...
      if (!targetContext) {
        throw new McpError(ErrorCode.InvalidParams, 
          `Cannot find ${contextType} context for extension ${extensionId}`);
      }
      ...
      const response: SwitchExtensionContextResponse = {
   ...
   * 分析所有扩展相关的Chrome目标
   */
  private analyzeExtensionTargets(allTargets: any[]): any[] {
   ...
   * 为单个扩展构建完整的上下文信息
   */
  private async buildExtensionContext(extensionId: string, targets: any[]): Promise<ExtensionContext> {
    // 获取扩展基本信息
    const manifestInfo = await this.getExtensionManifestInfo(extensionId);
    
    const contexts: ExtensionContexts = {
      contentScripts: [],
      devtools: []
    };
      ...
      if (target.type === 'service_worker' || target.type === 'background_page') {
        ...
        if (url.includes('/popup.html') || url.includes('popup')) {
          ...
          contexts.options = {
            ...
            url: url,
            open: true,
            tabId: (target as any).tabId
          };
        }
      } else if (target.type === 'devtools') {
        contexts.devtools!.push({
          targetId: target.targetId,
          inspectedTabId: (target as any).tabId || 'unknown',
          url: url
        });
      }
    });
   ...
   */
  private async detectContentScripts(extensionId: string): Promise<ExtensionContentScriptContext[]> {
    const contentScripts: ExtensionContentScriptContext[] = [];
   ...
   * 获取扩展的manifest信息
   */
  private async getExtensionManifestInfo(extensionId: string): Promise<{ name: string; manifestVersion: number }> {
    try {
   ...
   * 计算扩展的总上下文数量
   */
  private countContexts(contexts: ExtensionContexts): number {
    let count = 0;
    
    if (contexts.background) count++;
    count += contexts.contentScripts.length;
    if (contexts.popup) count++;
    if (contexts.options) count++;
    if (contexts.devtools) count += contexts.devtools.length;
  ...
  async findTargetContext(
    ...
    if (targetId) {
      const result = await cdpClient.Target.getTargets();
      const target = result.targetInfos.find((t: any) => t.targetId === targetId);
    ...
    for (const target of allTargets) {
      if (!target.url?.includes(`chrome-extension://${extensionId}`)) continue;
      ...
      switch (contextType) {
        ...
        case 'devtools':
          if (target.type === 'devtools') {
            return {
              ...
              targetId: target.targetId,
              url: target.url,
              tabId: (target as any).inspectedTabId
            };
          }
          break;
        ...
        case 'content_script':
          // 内容脚本需要通过tabId匹配
          if (tabId && target.type === 'page' && (target as any).tabId === tabId) {
   ...
   * 执行上下文切换
   */
  async performContextSwitch(target: ContextTarget): Promise<{ success: boolean; sessionId?: string }> {
    const cdpClient = this.chromeManager.getCdpClient();
    if (!cdpClient) {
   ...
   * 检测上下文能力
   */
  private async detectContextCapabilities(target: ContextTarget): Promise<{
    canEvaluate: boolean;
    ...
    const capabilities = {
      ...
      chromeAPIs: [] as string[]
    };

    if (!cdpClient) return capabilities;
