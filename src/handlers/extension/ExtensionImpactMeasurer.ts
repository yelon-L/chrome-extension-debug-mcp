...
export class ExtensionImpactMeasurer {
  private performanceAnalyzer: ExtensionPerformanceAnalyzer;
  private networkMonitor: ExtensionNetworkMonitor;
  
  // 默认影响级别阈值
  private readonly DEFAULT_THRESHOLDS: ImpactThresholds = {
   ...
   * 主方法：测量扩展的综合影响
   */
  async measureImpact(args: MeasureExtensionImpactArgs): Promise<ExtensionImpactReport> {
    ...
    try {
      ...
      for (const page of testPages) {
        ...
        for (let i = 1; i <= iterations; i++) {
          ...
          const result = await this.runSingleTest(
            ...
            performanceDuration,
            networkDuration,
            args.includeNetworkDetails || false
          );
      ...
      const report: ExtensionImpactReport = {
        extensionId,
        extensionName,
        testDate: Date.now(),
        configuration: {
   ...
   * 计算整体统计
   */
  private calculateOverallStats(pageResults: PageImpactSummary[]) {
  ...
  private calculateImpactScore(
    performance: PageImpactSummary['avgPerformance'],
    network: PageImpactSummary['avgNetwork']
  ): number {
    let score = 0;
   ...
   * 根据分数计算影响级别
   */
  private calculateImpactLevel(score: number): 'Critical' | 'High' | 'Medium' | 'Low' | 'Minimal' {
  ...
  private generateKeyFindings(
    ...
    overall: ExtensionImpactReport['overall']
  ): string[] {
   ...
   * 工具方法：标准化测试页面列表
   */
  private normalizeTestPages(pages: TestPage[] | string[]): TestPage[] {
   ...
   * 工具方法：获取扩展名称
   */
  private async getExtensionName(extensionId: string): Promise<string> {
