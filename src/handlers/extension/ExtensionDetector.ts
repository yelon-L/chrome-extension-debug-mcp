...
// 简化日志函数
const log = (...args: any[]) => console.error('[ExtensionDetector]', ...args);
...
export class ExtensionDetector {
  ...
  async getExtensionFullInfo(extensionId: string): Promise<any> {
    try {
      ...
      const bgTarget = targetInfos.find((t: any) => 
   ...
   * 获取扩展基本信息
   */
  async getExtensionInfo(extensionId: string): Promise<{name: string; version: string} | null> {
    try {
      const cdpClient = this.chromeManager.getCdpClient();
      if (!cdpClient) return null;
