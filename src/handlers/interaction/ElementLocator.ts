...
const log = (...args: any[]) => console.error('[ElementLocator]', ...args);
...
export class ElementLocator {
  private uidCache = new Map<string, ElementUID>();
  private stabilityMonitor = new Map<string, number>();
  ...
  async analyzeDOMStability(options: {
    ...
    try {
      ...
      while (Date.now() - startTime < monitorDuration) {
        ...
        snapshots.push({
          timestamp: (currentSnapshot as any).timestamp,
          elementCount: (currentSnapshot as any).elementCount,
          majorChanges: (currentSnapshot as any).majorChanges,
          changes
        });
      ...
      const totalChangeCount = Object.values(totalChanges).reduce((a, b) => (a as number) + (b as number), 0) as number;
   ...
   * 根据坐标获取元素
   */
  private async getElementAtCoordinates(x: number, y: number): Promise<string> {
    const page = await this.pageManager.getActivePage();
    if (!page) {
      throw new McpError(ErrorCode.InternalError, 'No active page available');
    }
   ...
   * 根据文本查找元素
   */
  private async findElementByText(textContent: string): Promise<string> {
    const page = await this.pageManager.getActivePage();
    if (!page) {
      throw new McpError(ErrorCode.InternalError, 'No active page available');
    }
   ...
   * 生成多种选择器策略
   */
  private async generateMultipleSelectors(targetElement: string, depth: number): Promise<SelectorGenerationResult['selectors']> {
    const page = await this.pageManager.getActivePage();
    if (!page) {
      throw new McpError(ErrorCode.InternalError, 'No active page available');
    }
    ...
    const result = await page.evaluate(script);
    return result as SelectorGenerationResult['selectors'];
  }

  /**
   * 分析选择器质量
   */
  private async analyzeSelectors(selectors: SelectorGenerationResult['selectors']): Promise<SelectorGenerationResult['analysis']> {
    const page = await this.pageManager.getActivePage();
    if (!page) {
      throw new McpError(ErrorCode.InternalError, 'No active page available');
    }
    ...
    const result = await page.evaluate(script);
    return result as SelectorGenerationResult['analysis'];
  }

  /**
   * 选择最佳选择器
   */
  private selectBestSelector(selectors: SelectorGenerationResult['selectors']): string {
   ...
   * 选择备用选择器
   */
  private selectBackupSelectors(selectors: SelectorGenerationResult['selectors'], recommended: string): string[] {
    return selectors
   ...
   * 生成稳定性建议
   */
  private generateStabilityRecommendations(changes: any, stability: number): string[] {
