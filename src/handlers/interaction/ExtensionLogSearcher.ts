...
const log = (...args: any[]) => console.error('[ExtensionLogSearcher]', ...args);
...
export class ExtensionLogSearcher {
   ...
   * 验证搜索选项
   */
  private validateSearchOptions(options: LogSearchOptions): void {
   ...
   * 收集日志数据
   */
  private async collectLogs(options: LogSearchOptions): Promise<ExtensionLogEntry[]> {
    ...
    if (options.extensionId) {
      filteredLogs = filteredLogs.filter(log => 
        this.isLogFromExtension(log, options.extensionId!)
      );
    } else if (options.extensionIds && options.extensionIds.length > 0) {
      filteredLogs = filteredLogs.filter(log => 
        options.extensionIds!.some(id => this.isLogFromExtension(log, id))
      );
    }
    ...
    if (options.sourceTypes && options.sourceTypes.length > 0) {
      filteredLogs = filteredLogs.filter(log => 
        options.sourceTypes!.includes(log.source || 'unknown')
      );
    }
   ...
   * 收集日志用于导出
   */
  private async collectLogsForExport(options: LogExportOptions): Promise<ExtensionLogEntry[]> {
    const searchOptions: LogSearchOptions = {
      query: '.*',
      useRegex: true,
      extensionId: options.extensionId,
   ...
   * 执行搜索
   */
  private async performSearch(logs: ExtensionLogEntry[], options: LogSearchOptions): Promise<LogSearchMatch[]> {
    ...
    for (const log of logs) {
      const matchDetails: LogSearchMatch['matchDetails'] = [];
   ...
   * 搜索文本中的内容
   */
  private searchInText(text: string, query: string, regex: RegExp | null, caseSensitive?: boolean): boolean {
    if (regex) {
  ...
  private getMatchDetails(
    ...
    field: 'message' | 'source' | 'url'
  ): LogSearchMatch['matchDetails'] {
    const details: LogSearchMatch['matchDetails'] = [];
   ...
   * 计算相关性分数
   */
  private calculateRelevanceScore(matchDetails: LogSearchMatch['matchDetails'], query: string): number {
    let score = 0;
    ...
    for (const detail of matchDetails) {
      ...
      // 字段权重
      switch (detail.field) {
        case 'message': score += 20; break;
        case 'source': score += 10; break;
        case 'url': score += 5; break;
      }
   ...
   * 获取控制台日志
   */
  private async getConsoleLogs(): Promise<ExtensionLogEntry[]> {
    // 这里应该调用现有的日志收集逻辑
   ...
   * 检查日志是否来自指定扩展
   */
  private isLogFromExtension(log: ExtensionLogEntry, extensionId: string): boolean {
    // 检查URL中是否包含扩展ID
   ...
   * 计算统计信息
   */
  private calculateStatistics(matches: LogSearchMatch[], allLogs: ExtensionLogEntry[]): LogSearchResult['statistics'] {
    const matchesByLevel: Record<string, number> = {};
    const matchesBySource: Record<string, number> = {};
    const matchesByExtension: Record<string, number> = {};
   ...
   * 格式化日志用于导出
   */
  private async formatLogs(logs: ExtensionLogEntry[], options: LogExportOptions): Promise<string> {
   ...
   * 格式化为CSV
   */
  private formatAsCSV(logs: ExtensionLogEntry[], includeMetadata?: boolean): string {
   ...
   * 格式化为文本
   */
  private formatAsText(logs: ExtensionLogEntry[], includeMetadata?: boolean): string {
    const lines: string[] = [];
   ...
   * 计算时间范围
   */
  private calculateTimeRange(logs: ExtensionLogEntry[]): { earliest: number; latest: number } {
    if (logs.length === 0) {
   ...
   * 提取日志模式
   */
  private extractPatterns(logs: ExtensionLogEntry[], minFrequency: number): LogPattern[] {
   ...
   * 计算级别比率
   */
  private calculateLevelRate(logs: ExtensionLogEntry[], level: string): number {
   ...
   * 生成时间线数据
   */
  private generateTimeline(logs: ExtensionLogEntry[]): LogPatternAnalysis['timeline'] {
    if (logs.length === 0) {
      return { interval: '1h', data: [] };
    }

    // 按小时分组
    const hourlyData = new Map<number, { count: number; errorCount: number; warningCount: number }>();
    ...
    for (const log of logs) {
      ...
      if (log.level === 'error') data.errorCount++;
      if (log.level === 'warning') data.warningCount++;
    }
