...
export class MutexUsageExample {
   ...
   */
  static async basicMutexUsage(operationName: string) {
   ...
   * 展示在工具执行中如何使用Mutex
   */
  static async executeToolWithMutex(toolName: string, args: any) {
    const guard = await this.mutex.acquire();
    const startTime = Date.now();
  ...
  static async demonstrateErrorHandling() {
    ...
    try {
      ...
      console.error(`❌ [ErrorDemo] Caught error:`, (error as Error).message);
      // 即使出错，也要确保释放锁
      throw error;
    } finally {
      const duration = Date.now() - startTime;
  ...
  // 辅助方法 - 模拟Chrome操作
  private static async simulateChromeOperation(name: string): Promise<void> {
    const duration = 500 + Math.random() * 1000; // 0.5-1.5秒
    console.log(`  ⚙️  [${name}] Executing Chrome operation...`);
    await new Promise(resolve => setTimeout(resolve, duration));
  }
  
  private static async simulateAttachToChrome(args: any) {
  ...
  private static async simulateListExtensions(args: any) {
  ...
  private static async simulateEvaluate(args: any) {
